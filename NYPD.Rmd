...
Let's see if we can create a model that can tell us something about the dates and times of the incidents.
...
This is obviously not a particularly useful model, as the scatterplot points are vertically polarized, and the correlation coefficient is likely zero. However, this model can help us to see that there was *some* (even if negligible) change in time of day as the years went on, with more incidents occurring in the evening rather than in the early morning. That being said, the model is mostly flat, which tells us that there was not much (if any) change in time of day through the years.

Let's see if we can build a more useful model, this time utilizing the total number of deaths each year.
...
murders_by_year %>% 
  ...
  geom_smooth(alpha = 0, color = "black") +
  theme_bw() +
...
Here, we have visualized the total number of deaths caused by shooting incidents each year. Our model is extremely helpful this time. We can see that deaths were high in 2006, declined until about 2016, and then increased sharply in 2020.
